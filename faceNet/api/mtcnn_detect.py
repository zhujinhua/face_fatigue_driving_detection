...
from faceNet.api import nets, tool
...
def crop_detected_boxes(image_path, detector, target_size=(48, 48),image_data=None):
    if image_data:
    ...
    if len(final_box) == 0:
        return torch.empty(0), []
    x1 = int(final_box[0]) if final_box[0] >= 0 else 0
    y1 = int(final_box[1]) if final_box[1] >= 0 else 0
    x2 = int(final_box[2]) if final_box[2] >= 0 else 0
    y2 = int(final_box[3]) if final_box[3] >= 0 else 0
    ...
    return cropped_img_tensor, final_box
...
def get_detect_face(img_path=None, target_size=(48, 48), image_data=None):
    ...
    return crop_detected_boxes(img_path, detector, target_size, image_data=image_data)
