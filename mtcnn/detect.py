...
class Detector:
    def __init__(self, pnet_path, rnet_path, onet_path, softnms=False, thresholds=None, factor=0.709):
        ...
        self.pnet.load_state_dict(torch.load(pnet_path, map_location=device), strict=False)
        self.rnet.load_state_dict(torch.load(rnet_path, map_location=device), strict=False)
        self.onet.load_state_dict(torch.load(onet_path, map_location=device), strict=False)
    ...
    def pnet_detect(self, image):
        ...
        while min_side > 12:
            ...
            # 找到置信度大于0.6的索引，根据索引判断建议框在原图的位置，再根据建议框求出真实框
