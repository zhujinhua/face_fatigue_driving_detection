...
class Trainer:
    ...
    def train_bak(self, stop_value, landmark=False):
        """
            - 断点续传 --> 短点续训
        ...
        while True:
            ...
            for i, (img_data, _cls, _offset, _point) in enumerate(dataloader):
                ...
                if landmark:
                    ...
                    print("epoch: {0}, loss:{1:.4f}, cls_loss:{2:.4f}, offset_loss:{3:.4f}, point_loss:{4:.4f}".format(
                        epochs, loss.float(), cls_loss.float(), offset_loss.float(), point_loss.float()))
                else:
                    # dataloader.set_description(
                    #     "epochs:{}, loss:{:.4f}, cls_loss:{:.4f}, offset_loss:{:.4f}".format(
                    #         epochs, loss.float(), cls_loss.float(), offset_loss.float()))
                    print("epoch: {0}, loss:{1:.4f}, cls_loss:{2:.4f}, offset_loss:{3:.4f}".format(
                        epochs, loss.float(), cls_loss.float(), offset_loss.float()))
